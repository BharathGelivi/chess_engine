    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <aside className="analysis-panel">
          {mode === 'analysis' ? (
            <>
              <div className="panel-heading"><div><span className="label">POSITION</span><h2>{viewGame.isCheckmate() ? 'Checkmate' : viewGame.isCheck() ? 'King in check' : 'Quiet position'}</h2></div><div className="eval">{engineState.evaluation}</div></div>
            ...
            <>
              <div className="panel-heading"><div><span className="label">ENGINE VS ENGINE</span><h2>{autoplay.result || (autoplay.loading ? 'Loading engines...' : autoplay.running ? 'Playing...' : 'Ready')}</h2>{autoplay.loading && <p className="empty">Downloading {engineNames[autoplay.white]} and {engineNames[autoplay.black]}... first load can take a couple of minutes.</p>}</div></div>
